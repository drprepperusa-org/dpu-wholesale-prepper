...
function OrderConfirmModal({ isOpen, onClose, onSubmit, cartItems = [], total = 0, isSubmitting = false }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[10001] p-5"
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-[400px] w-full overflow-hidden max-sm:rounded-xl max-sm:max-w-full"
        style={{ animation: 'slideUp 0.3s cubic-bezier(.32,1,.32,1)' }}
        onClick={(e) => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <h3 className="text-[17px] font-semibold text-slate-800 m-0">Confirm Order</h3>
        ...
        {/* Body */}
        <div className="p-5">
          <p className="text-[13px] text-slate-500 mb-4 leading-relaxed">You&apos;re about to place an order. Please review the details below:</p>

          <div className="bg-slate-50 border border-slate-200 rounded-lg p-3.5 mb-4 text-[13px]">
        ...
        <div className="flex gap-2.5 px-5 py-4 border-t border-slate-200">
          <button onClick={onClose} disabled={isSubmitting}
            className="flex-1 py-3 rounded-lg border border-slate-200 bg-slate-50 text-slate-800 font-semibold text-[13px] cursor-pointer transition-colors hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed">
            Cancel
          </button>
          <button onClick={onSubmit} disabled={!confirmTerms || isSubmitting}
            className="flex-1 py-3 rounded-lg border border-indigo-500 bg-indigo-500 text-white font-semibold text-[13px] cursor-pointer transition-colors hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Placing...</> : 'Place Order'}
          </button>
        </div>
      </div>
    </div>
