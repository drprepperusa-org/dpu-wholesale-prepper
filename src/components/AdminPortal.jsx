        ...
        <div className="categories-main">
          ...
          <div className="cat-section">
            ...
            <div className="cat-list-wrap">
              <div className="cat-drag-list">
                {superCategoriesList.map((element, index) => (
                  <div key={element.id} className="cat-item cat-super-item">
              ...
              <div className="cat-list-wrap">
                <div className="cat-drag-list">
                  {(categoriesBySuper[superCat.id] || []).map((element, index) => (
                    <div key={element.id} className="cat-item">
                      <div className="cat-drag-handle">{'\u22EE\u22EE'}</div>
                      <div className="cat-item-content">
                        <span className="cat-item-name">{element.name}</span>
                        <span className="cat-item-order">Position: {index + 1}</span>
      ...
      <div className={`modal-wrap ${activeModal === 'addProdModal' ? 'open' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
        <div className="modal">
          ...
          <select
            ...
            onChange={e => { setNewProductForm(prev => ({ ...prev, category_id: e.target.value })); setProductFormErrors(prev => { const n = { ...prev }; delete n.category_id; return n; }); }}
          >
          ...
          {editingProduct && (
            <div className="edit-field-grid">
              ...
              <div className="edit-field">
                ...
                <input type="text" value={editingProduct.category || ''} onChange={e => setEditingProduct(prev => ({ ...prev, category: e.target.value }))} />
              </div>
              <div className="edit-field">
              ...
              <div className="edit-field">
                <label className="form-checkbox">
                  <input type="checkbox" checked={!!editingProduct.show_price} onChange={e => setEditingProduct(prev => ({ ...prev, show_price: e.target.checked }))} />
                  Show price on cards
                </label>
              </div>
...
.search-box { display: flex; align-items: center; gap: 8px; padding: 7px 12px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; box-shadow: var(--shadow); }
...
.btn-add-prod { padding: 8px 16px; background: var(--red); border: none; border-radius: 8px; color: #fff; font-family: 'DM Sans', sans-serif; font-weight: 600; font-size: 13px; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
...
.card-select-check input[type="checkbox"] { width: 16px; height: 16px; cursor: pointer; accent-color: var(--red); }
.card-badges { display: flex; gap: 4px; margin-bottom: 8px; flex-wrap: wrap; }
...
.cat-drag-handle { color: var(--muted); font-size: 14px; flex-shrink: 0; user-select: none; }
...
.form-checkbox input[type="checkbox"] { width: 18px; height: 18px; cursor: pointer; accent-color: var(--red); }
